...
const Keys: {
	MODIFIER_KEYS: typeof Keys_["MODIFIER_KEYS"],
	KEY_MODS: typeof Keys_["KEY_MODS"] & {[key: number]: number | string},
	FUNCTION_KEYS : typeof Keys_["FUNCTION_KEYS"] & {[key: string]: string},
	PRINTABLE_KEYS: typeof Keys_["PRINTABLE_KEYS"] & {[key: string]: string},
} & { [key in string]: string | number | object } = { ...Keys_ };
...
const codeToKeyCode: (typeof codeToKeyCode_) & {[key: string]: number} = { ...codeToKeyCode_ };
...
for (var mod in modifiers) {
	codeToKeyCode[mod] = codeToKeyCode[mod + "Left"]
		= codeToKeyCode[mod + "Right"] = modifiers[mod as keyof typeof modifiers];
}
export const $codeToKeyCode = codeToKeyCode;
...
// variables as well.
Object.assign(Keys, Keys.MODIFIER_KEYS);
Object.assign(Keys, Keys.PRINTABLE_KEYS);
Object.assign(Keys, Keys.FUNCTION_KEYS);
...
(function() {
	...
	for (var i = Math.pow(2, mods.length); i--;) {
		Keys.KEY_MODS[i] = mods.filter(function(x) {
			return i & (Keys.KEY_MODS as any)[x];
		}).join("-") + "-";
	}
})();
...
export function keyCodeToString(keyCode: number): string {
	...
	var keyString = (Keys as any)[keyCode];
